...
export function observeTool<A extends unknown[], R extends ToolResult>(tool: string, handler: (...args: A) => Promise<R>) {
  return async (...args: A): Promise<R> => {
    ...
    console.log({ event: 'mcp_tool_start', requestId, tool });
    try {
      ...
      console.log({ event: 'mcp_tool_end', requestId, tool, durationMs: Math.round(performance.now() - started),
        outcome: result.isError ? 'error' : failedItems ? 'partial' : 'success', errorCodes,
        ...(items.length ? { itemCount: items.length, failedItems } : {}),
      });
      return result;
    } catch (error) {
      console.log({ event: 'mcp_tool_end', requestId, tool, durationMs: Math.round(performance.now() - started), outcome: 'exception' });
      throw error;
    }
  };
}
