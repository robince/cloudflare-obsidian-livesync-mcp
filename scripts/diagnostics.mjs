...
export async function capture(workers,account,duration,sanitize,events,spawnChild=spawn) {
  const bin=await wranglerPath(), children=[],sources={};let interrupted=false, stopping=false, killTimer;
  const terminate=()=>{stopping=true;for(const child of children)child.kill('SIGTERM');killTimer??=setTimeout(()=>{for(const child of children)child.kill('SIGKILL')},5000)};
  const stop=()=>{interrupted=true;terminate()};process.once('SIGINT',stop);
  const timer=setTimeout(terminate,duration*1000);
  try {
    await Promise.all(workers.map(worker=>new Promise(resolveChild=>{
      ...
      child.stdout.setEncoding('utf8');child.stdout.on('data',chunk=>{try{parser.push(chunk)}catch{sources[key]={status:'partial',reason:'tail_json_or_event_limit'};terminate()}});
      child.stderr.resume();
      const finish=(code,signal)=>{if(ended)return;ended=true;if(sources[key].status==='collected'){try{parser.end()}catch{sources[key]={status:'partial',reason:'incomplete_tail_json'}}if(!stopping || (code!==0&&!signal))sources[key]={status:'partial',reason:'tail_failed_check_worker_and_permissions'};if(!received && sources[key].status==='collected')sources[key]={status:'partial',reason:'no_events_connection_unconfirmed'};}resolveChild()};
...
export async function main(args=process.argv.slice(2)) {
  ...
  try {
    const api=apiClient(await credentials());
    const account=await resolveAccount(api,config.account);
    if(config.command==='capture') { console.log('Starting live capture. Connection is confirmed when the first event arrives; reproduce the problem during the capture window.'); Object.assign(sources,await capture(config.workers,account,config.duration,sanitize,events)); }
    else {
